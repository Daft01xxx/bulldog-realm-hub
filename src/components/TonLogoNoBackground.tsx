...
interface TonLogoNoBackgroundProps {
  className?: string;
  size?: "sm" | "md" | "lg" | "xl";
}

export default function TonLogoNoBackground({ className, size = "md" }: TonLogoNoBackgroundProps) {
  const sizeClasses = {
    sm: "w-6 h-6",
    md: "w-8 h-8", 
    lg: "w-10 h-10",
  ...
  return (
    <div className={cn("inline-flex items-center", className)}>
      ...
      >
        {/* Just the diamond shape without background circle */}
        <path 
          ...
          fill="url(#tonGradientNoBackground)" 
        />
        <defs>
